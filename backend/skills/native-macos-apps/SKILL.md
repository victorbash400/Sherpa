...
6. Observe again after other meaningful actions and compare the state. If a receipt is stale, discard it and capture a fresh observation rather than retrying the old element.
7. Stop when the requested state is visibly verified. Do not quit, relaunch, close windows, or affect every application unless the user explicitly asks.
