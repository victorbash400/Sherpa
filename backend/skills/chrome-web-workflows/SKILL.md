...
description: Navigate and operate connected Chrome tabs, including efficient repetitive DOM edits, with bounded snapshots and verification. Use for explicit websites, existing browser sessions, and web-only workflows.
---

# Chrome workflow

1. Begin with `browser_tabs`; reuse the relevant existing tab when possible. Use one bounded `browser_snapshot` before acting, or `browser_find` for one target in a large page.
...
4. After a programmatic edit, navigation, typing, selection, or submission, use `browser_find` or a bounded snapshot to verify the expected visible state. Do not alternate full snapshots with every keystroke.
